...
パーリ語の主要な経典を題材としたテキスト読解と音声読み上げを中心とした学習サイト。各単語にローマ字読み、カタカナフリガナ、和訳、英訳を表示し、将来的にタイ語など他言語への拡張も可能な構造で実装します。ミニマルなデザインで集中して学習できる環境を提供します。
...
│   ├── WordTooltip.tsx         # 単語詳細情報ツールチップ（ローマ字、カタカナ、和訳、英訳）
...
```typescript
{
  ...
  romanization?: string;  // ローマ字読み（テキスト全体）
...
2. **Phase 2: テキスト読解機能**
   ...
   - テキスト表示コンポーネント
   - ローマ字読みとカタカナフリガナの表示機能
   - 単語の詳細情報ツールチップ（ローマ字、カタカナ、和訳、英訳）
   - 和訳の表示/非表示切り替え機能
   - 基本的なスタイリング
...
- **ローマ字・カタカナ変換**: パーリ語からローマ字読みとカタカナフリガナへの自動変換機能を実装（必要に応じて辞書データに保存）
